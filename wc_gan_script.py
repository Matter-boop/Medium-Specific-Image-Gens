...
with open("wc_array.pkl", "rb") as f:
    wc_ds = pickle.load(f)
...
#Builds dismcriminator for watercolour

#discriminator = build_discriminator()

discriminator = load_model("discriminator_model_wc_40000.h5")
discriminator.load_weights("discriminator_weights_wc_40000.weights.h5")
...
#Combines model for wc

#generator = build_generator() 

generator = load_model("generator_model_wc_40000.h5")
generator.load_weights("generator_weights_wc_40000.weights.h5")
...
X = wc_ds #Array of images in decimal form
print(X.shape)
...
for epoch in range(num_epochs): 
    ...
    if epoch % 1000 == 0:
        ...
        generator.save("generator_model_wc_fin" + str(epoch+starter_epoch) + ".h5")
        generator.save_weights("generator_weights_wc_fin" + str(epoch+starter_epoch) + ".weights.h5")
        discriminator.save("discriminator_model_wc_fin" + str(epoch+starter_epoch) + ".h5")
        discriminator.save_weights("discriminator_weights_wc_fin" + str(epoch+starter_epoch) + ".weights.h5")
...
for i in range(10):
    epoch = 41000 + i*1000
    generator = load_model("generator_model_wc_fin" + str(epoch) + ".h5")
    generator.load_weights("generator_weights_wc_fin" + str(epoch) + ".weights.h5")
    # discriminator = load_model("discriminator_model_wc_fin" + str(64000 + i*1000) + ".h5")
    # discriminator.load_weights("discriminator_weights_wc_fin" + str(64000 + i*1000) + ".weights.h5")
    ...
    #Calculating scores
    calculated_fid = calculate_fid(wc_ds[:200],fake_images)
    fake_image = (fake_images[0] + 1) * 127.5
    ssim_skimg = ssim(wc_ds[0], fake_image, channel_axis=-1, data_range=255)
    ...
    LPIPS_scores.append(loss_fn_alex(prepare_for_lpips(wc_ds[0:1]), prepare_for_lpips(fake_images[0:1])).item())
...
#Openning pickle
import pickle
with open("wc_scores.pkl", "rb") as f:
    data = pickle.load(f)
...
# %%
with open("wc_scores.pkl", "wb") as f:
    pickle.dump({
...
# %%
#Combined metric showcasing both watercolour and photo scores
import pickle

with open("wc_scores.pkl", "rb") as f:
    data = pickle.load(f)

FID_scores_wc = data["FID_scores"]
SSIM_scores_wc = data["SSIM_scores"]
LPIPS_scores_wc = data["LPIPS_scores"]
epochs_to_test_wc = data["epochs"]

with open("photo_scores.pkl", "rb") as f:
    data = pickle.load(f)

FID_scores_photo = data["FID_scores"]
SSIM_scores_photo = data["SSIM_scores"]
LPIPS_scores_photo = data["LPIPS_scores"]
epochs_to_test_photo = data["epochs"]

def combine_scores(FID, SSIM, LPIPS):
    combined_scores = []
    #normalise metrics where 1 is best and 0 is worst

    FID_norm = [1 - (fid - min(FID)) / (max(FID) - min(FID)) for fid in FID]
    SSIM_norm = SSIM
    LPIPS_norm = [-(lpips -1) for lpips in LPIPS]
    
    for i in range(len(FID)):
        combined_score = (FID_norm[i] + SSIM_norm[i] + LPIPS_norm[i]) / 3
        combined_scores.append(combined_score)

    return combined_scores,FID_norm, SSIM_norm, LPIPS_norm

# %%
import matplotlib.pyplot as plt

wc_combined_scores, FID_norm_wc, SSIM_norm_wc, LPIPS_norm_wc = combine_scores(FID_scores_wc, SSIM_scores_wc, LPIPS_scores_wc)
photo_combined_scores, FID_norm_photo, SSIM_norm_photo, LPIPS_norm_photo = combine_scores(FID_scores_photo, SSIM_scores_photo, LPIPS_scores_photo)

fig, axs = plt.subplots(2, 1, figsize=(8,10))

axs[0].plot(epochs_to_test_wc, wc_combined_scores, marker='.')
axs[0].plot(epochs_to_test_wc, FID_norm_wc, marker='.')
axs[0].plot(epochs_to_test_wc, SSIM_norm_wc, marker='.')
axs[0].plot(epochs_to_test_wc, LPIPS_norm_wc, marker='.')

axs[0].set_title("Watercolour Composite Scores")
axs[0].set_ylabel("Score")
axs[0].set_xlabel("Epoch")
axs[0].set_ylim(0, 1)
axs[0].margins(x=0, y=0)
axs[0].grid()
axs[0].legend(["Combined Score", "Normalised Inverted FID", "SSIM", "InvertedLPIPS"], loc="upper left")

axs[1].plot(epochs_to_test_photo, photo_combined_scores, marker='.')
axs[1].plot(epochs_to_test_photo, FID_norm_photo, marker='.')
axs[1].plot(epochs_to_test_photo, SSIM_norm_photo, marker='.')
axs[1].plot(epochs_to_test_photo, LPIPS_norm_photo, marker='.')

axs[1].set_title("Photo Composite Scores")
axs[1].set_ylabel("Score")
axs[1].set_xlabel("Epoch")
axs[1].set_ylim(0, 1)
axs[1].margins(x=0, y=0)
axs[1].grid()
axs[1].legend(["Combined Score", "Normalised Inverted FID", "SSIM", "Inverted LPIPS"], loc="upper left")

plt.tight_layout()
plt.show()

# %%
#Mann-Whitney U test to compare the scores of the two models
import scipy.stats as stats

#FID U test
print("FID U test:")
print(stats.mannwhitneyu(FID_norm_wc, FID_norm_photo, alternative='two-sided'),"\n")

#LPIPS U test
print("LPIPS U test:")
print(stats.mannwhitneyu(LPIPS_norm_wc, LPIPS_norm_photo, alternative='two-sided'),"\n")

#SSIM U test
print("SSIM U test:")
print(stats.mannwhitneyu(SSIM_norm_wc, SSIM_norm_photo, alternative='two-sided'),"\n")

#Combined score U test
print("Combined Score U test:")
print(stats.mannwhitneyu(wc_combined_scores, photo_combined_scores, alternative='two-sided'),"\n")

#two sided because we are comparing differences
# %%
